 ...
 */
var atStream = client.stream("statuses/filter", { track: "@Bratwurst_bot" });
atStream.on("tweet", function (tweet) {
	var start = new Date().getTime(),
		message = tweet.text.toLowerCase(),
		tweetID = tweet.id_str,
		username = tweet.user.screen_name,
		ignored = isIgnored(username),
		test,
		params = {
	...
	if (username != "Bratwurst_bot" && message.indexOf("rt @") < 0) {
		...
		if (message.indexOf("@bratwurst_bot") == 0) {
			if (message.indexOf("ignore me") == 15) {
				...
				// Notice user
				var val = IGNORE.splice(IGNORE.indexOf(username), 1);
				if (val[0] == username) {
				...
				// Return Help
				var help = fs.readFileSync("help.jpg", { encoding: "base64" });

				params.status += "There you go";
				params.status += " [" + (new Date().getTime() - start) + "ms]";
				earlier = true;
				client.post("media/upload", { media_data: help }, function (err, data, response) {
		...
		if (message.indexOf("@bratwurst_bot random image") > -1) {
			...
			// Random Image
			var images = fs.readdirSync("bratwursts"),
				index = Math.floor(Math.random() * images.length),
				image = fs.readFileSync("bratwursts/" + images[index], { encoding: "base64" });

			params.status += "Have a bite";
			params.status += " [" + (new Date().getTime() - start) + "ms]";
			earlier = true;
			client.post("media/upload", { media_data: image }, function (err, data, response) {
				if (err) return log("ERROR:", err);
			...
			var rank = [],
				rankAt;
			for (var i = 0; i < STATS.length; i++) rank.push(STATS[i].array.length);
			rank.sort();
			rank.reverse();

			if (exists) {
				rankAt = rank.indexOf(STATS[exists].array.length) + 1;
				params.status += "In der Uptime von v2.0+ hast du " + STATS[exists].array.length + " mal über Bratwurst getwittert.\n";
